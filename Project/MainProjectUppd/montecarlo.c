...
int main(int argc, char **argv){
	...
	if(rank == 0){
		...
		MPI_Alloc_mem(2*size*sizeof(int), MPI_INFO_NULL, &minmax);
		MPI_Win_create(minmax, 2*size*sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &minmaxwin);
	}
	else{
		...
		MPI_Win_create(NULL, 0, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &minmaxwin);
	}
	...
	MPI_Win_fence(MPI_MODE_NOPRECEDE, minmaxwin);
	...
	for(int iter = 0; iter<n; iter++){
		...
		if(x[0] < local_minmax[0]) local_minmax[0] = x[0];
		if(x[0] > local_minmax[1]) local_minmax[1] = x[0];
	...
	//Sending the local min and max values of x[0] to rank 0
	MPI_Put(local_minmax, 2, MPI_INT, 0, rank*2, 2, MPI_INT, minmaxwin);
	...
	MPI_Win_fence(MPI_MODE_NOSUCCEED, minmaxwin);
	...
	if(rank == 0){
		...
		//getting the bin limits
		int min_bin = minmax[0];
		int max_bin = minmax[1];
		for(int i = 2; i<(size*2); i+=2){
			if(min_bin>minmax[i]) min_bin = minmax[i];
			if(max_bin<minmax[i+1]) max_bin = minmax[i+1]; 
		}



		//Write the output file
        if (0 != write_output(output_name, result, num_experiments, min_bin, max_bin)) {
            return 2;
		}
	}
...
int write_output(const char *file_name, int *result, int num_experiments, int min_bin, int max_bin) {
	FILE *file;
	...
	int bin_steps = (max_bin-min_bin)/20;
	int current_bin = min_bin;
	for(int i = 0; i<20; i++){
		if (0 > fprintf(file, "%d ", current_bin)) {
			perror("Couldn't write to output file");
		}
		current_bin += bin_steps;
	}
	if (0 > fprintf(file, "%d ", max_bin)) {
		perror("Couldn't write to output file");
	}
	if (0 > fprintf(file, "\n")) {
		perror("Couldn't write to output file");
